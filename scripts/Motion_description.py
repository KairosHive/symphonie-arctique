...
to produce a short motion prompt for an image-to-video model.

Usage:
  python img2vid_motion_prompts.py \
    --root /path/to/images \
    --out prompts.jsonl \
    --model gemma3:8b

Optional:
  --sidecar            # also write <image>.motion.txt next to each image
  --extensions jpg,jpeg,png,webp,bmp,tif,tiff,gif
  --temperature 0.2 --top-p 0.9 --num-predict 120 --seed 0
...
USER_INSTRUCTION = (
    "From this image, produce a concise motion description for an image-to-video model. "
...
def ask_model_for_motion_prompt(
        *,
        client,
        model: str,
        image_path: Path,
    ...
    resp = client.chat(
        ...
        messages=messages,
        options=options,
        format=OUTPUT_SCHEMA,   # enforce {"motion_prompt": "..."}
        stream=False,
        keep_alive="20m",
    )
    ...
    # ---- Parse JSON (schema response should be a JSON object with "motion_prompt") ----
    if content:
        # try direct JSON
        try:
            data = json.loads(content)
            if isinstance(data, dict) and isinstance(data.get("motion_prompt"), str):
                return data["motion_prompt"].strip()
        except Exception:
...
def write_sidecar_text(img_path: Path, text: str):
    sidecar = img_path.with_suffix(img_path.suffix + ".motion.txt")
    sidecar.write_text(text, encoding="utf-8")

def main():
    ap = argparse.ArgumentParser(description="Generate motion prompts from images using Gemma3 (Ollama).")
    ...
    # Write a small header line if file is new (optional; JSONL stays line-based)
    ...
    for idx, img in enumerate(images, start=1):
        ...
        try:
            motion = ask_model_for_motion_prompt(
                client=client,
                model=args.model,
                image_path=img,
            ...
            rec = {
                ...
                "image_name": image_name,   # <-- adds the filename only
                "image_rel": image_rel,     # <-- path relative to --root (nice for datasets)
                "model": args.model,
